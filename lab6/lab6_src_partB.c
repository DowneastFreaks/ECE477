...
void init_gpio(void);
int read_adc(void);
void toggle_leds(unsigned char);
...
void main(void)
{
    ...
    while(1)
    {
        ...
        printf("The power rail is approximately %.6fV, there are %d LEDs on.\n\r", (0x400 * 1.1)/ adc_voltage, led_count);
...
void toggle_leds(unsigned char val)
{
    if (val <= 3)
    {
        PORTB = val;
